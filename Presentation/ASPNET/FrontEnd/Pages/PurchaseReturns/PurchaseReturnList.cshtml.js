                    ...
                    columns: [
                        ...
                        //},
                        {
                            ...
                            edit: {
                                ...
                                read: () => typeof pluObj !== 'undefined' ? pluObj?.value : "",
                                destroy: () => { if (typeof pluObj !== 'undefined') pluObj?.destroy(); },
                                ...
                                write: (args) => {
                                    ...
                                    const applyProductToObjs = (productId, product, qty = 1) => {
                                        ...
                                        if (typeof productObj !== 'undefined' && productObj) {
                                            productObj.value = productId;
                                            productObj.dataBind();
                                        }
                                        if (typeof orderQuantityObj !== 'undefined' && orderQuantityObj) {
                                            orderQuantityObj.value = stock;
                                            orderQuantityObj.readOnly = true;
                                        }
                                        if (typeof returnQuantityObj !== 'undefined' && returnQuantityObj) {
                                            returnQuantityObj.value = qty;
                                        }

                                        args.rowData.productId = productId;
                                        args.rowData.orderQuantity = stock;
                                        args.rowData.maxReturnQty = stock;
                                        args.rowData.returnQuantity = qty;
                                    };

                                    // ── HELPER B: Open attribute modal + auto-add next row ───
                                    const openAttributeModalWithAutoNext = async (rowData) => {
                                        let rowIndex = state.secondaryData.findIndex(r => r === rowData || (r.id && r.id === rowData.id));
                                        let injected = false;
                                        ...
                                        // Pass both rowIndex and rowData
                                        await methods.openDetailModal(rowIndex, rowData);
                                    ...
                                    const processPLU = async (enteredPLU) => {
                                        ...
                                        try {
                                            ...
                                            // 🔥 Stock check for GRN constraints
                                            const availableStock = methods.getProductStock?.(productId) || 0;
                                            if (availableStock <= 0) {
                                                Swal.fire({
                                                    icon: 'warning',
                                                    title: 'Not in Goods Receive',
                                                    text: 'This product has no available stock to return from the selected GRN.',
                                                    timer: 3500,
                                                    showConfirmButton: false
                                                });
                                                return;
                                            ...
                                            // ── Duplicate check directly against state.secondaryData ──
                                            const duplicateRow = state.secondaryData.find(r => r.productId === productId);

                                            if (duplicateRow) {
                                                // ════════════════ DUPLICATE PATH ═════════════════
                                                const currentQty = parseFloat(duplicateRow.returnQuantity) || 0;
                                                const newQty = currentQty + 1;

                                                // Cancel the current empty row creation since we're updating an existing row
                                                secondaryGrid.obj.closeEdit();

                                                // Guard: Prevent scanning more than available in GRN
                                                if (newQty > availableStock) {
                                                    Swal.fire({
                                                        icon: 'warning',
                                                        title: 'Max Return Quantity',
                                                        text: `You cannot return more than ${availableStock} units of this product.`,
                                                        timer: 3000,
                                                        showConfirmButton: false
                                                    });
                                                    return;
                                                ...
                                                console.log('♻️ Duplicate detected, incrementing quantity natively');

                                                // Update state directly
                                                duplicateRow.returnQuantity = newQty;
                                                secondaryGrid.obj.refresh(); // Refresh grid UI to reflect new quantity

                                                if (methods.refreshSummary) methods.refreshSummary();
                                                ...
                                                // Force blur to sync value with Syncfusion Grid's validation
                                                inputElement.blur();
                                                inputElement.dispatchEvent(new Event('change', { bubbles: true }));

                                                // Set rowData and apply to UI components
                                                args.rowData.pluCode = enteredPLU;
                                                applyProductToObjs(productId, product, 1);

                                                const committedRow = args.rowData;

                                                // Delay to allow Syncfusion validation state to securely update
                                                setTimeout(() => {
                                                    // Commit the row
