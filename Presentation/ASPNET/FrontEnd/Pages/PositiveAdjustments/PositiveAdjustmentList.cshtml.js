                            ...
                            edit: {
                                ...
                                write: (args) => {
                                    ...
                                    const applyProductToObjs = (productId, product, qty = 1) => {
                                        ...
                                        args.rowData.productId = productId;
                                        args.rowData.quantity = qty; // <--- Check this field name

                                        // If you track cost or totals in Positive Adjustment, set them here:
                                        // args.rowData.unitCost = product?.unitCost ?? 0;
                                        // args.rowData.total = qty * (product?.unitCost ?? 0);
                                    };

                                    // ── HELPER B: Recalculate persisted row data ─────────────
                                    const recalcRowData = (rowData, productId, newQty) => {
                                        rowData.quantity = newQty; // <--- Check this field name
                                        // rowData.total = newQty * (rowData.unitCost ?? 0);
                                    };
                                    ...
                                    const openAttributeModalWithAutoNext = async (rowData) => {
                                        ...
                                        // 🔥 FIX: Passed rowData as the second argument here
                                    ...
                                    const processPLU = async (enteredPLU) => {
                                        ...
                                        try {
                                            ...
                                            // ── Duplicate check ───────────────────────────────
                                            // Assuming you are using manualBatchChanges here as well
                                            const allGridData = [
                                                ...state.secondaryData,
                                            ...
                                            if (duplicateRow) {
                                                ...
                                                // Grab current quantity, default to 0 if undefined, then add 1
                                                const newQty = (parseFloat(duplicateRow.quantity) || 0) + 1;
                                                ...
                                                setTimeout(() => {
                                                    ...
                                                    // 4. Wait for grid's save cycle to finish fully
                                                    setTimeout(() => {
                                                        if (hasAttributes) {
